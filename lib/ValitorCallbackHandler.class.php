...
class ValitorCallbackHandler
{
     ...
     *
     * @return ValitorAbstractPaymentResponse
     */
    public function parseXmlResponse($xml)
    {
        ...
        case 'subscription':
        case 'verifyCard':
            return new ValitorOmniReservationResponse($xml);
        case 'subscriptionAndCharge':
        case 'recurringAndCapture':
            return new ValitorCaptureRecurringResponse($xml);
        default:
    ...
    private function verifyXml(SimpleXMLElement $xml)
    {
        if ($xml->getName() != 'APIResponse') {
            throw new ValitorXmlException('Unknown root-tag <'.$xml->getName().'> in XML, should have been <APIResponse>', $xml);
        }
        if (!isset($xml->Header)) {
            throw new ValitorXmlException('No <Header> in response', $xml);
        }
        if (!isset($xml->Header->ErrorCode)) {
            throw new ValitorXmlException('No <ErrorCode> in Header of response', $xml);
        }
        if ((string)$xml->Header->ErrorCode !== '0') {
            throw new Exception($xml->Header->ErrorMessage.' (Error code: '.$xml->Header->ErrorCode.')');
        }
        if (!isset($xml->Body)) {
            throw new ValitorXmlException('No <Body> in response', $xml);
        }

        if ($this->getBodyResult($xml) != 'Error' && !isset($xml->Body[0]->Transactions)) {
            $error = $this->getBodyMerchantErrorMessage($xml);
            throw new ValitorXmlException('No <Transactions> in <Body> of response'.($error ? ' ('.$error.')' : ''), $xml);
        }
        if ($this->getBodyResult($xml) != 'Error' && !isset($xml->Body[0]->Transactions[0]->Transaction)) {
            $error = $this->getBodyMerchantErrorMessage($xml);
            throw new ValitorXmlException('No <Transaction> in <Transactions> of response'.($error ? ' ('.$error.')' : ''), $xml);
        }
        if ($this->getBodyResult($xml) == 'Error' && !isset($xml->Body[0]->Transactions[0]->Transaction)) {
            $error = $this->getBodyMerchantErrorMessage($xml);
            throw new ValitorXmlException(($error ? ' ('.$error.')' : ''), $xml);
        }
    }
